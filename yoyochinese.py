...
'''
Download sound files from yoyochinese and create Anki cards

https://www.yoyochinese.com/audio/pychart/e2.mp3
'''
...
class YoyoLink(pinyin.PinyinLink):
    def pinyin(self):
        return self._pinyin
    ...
    def __init__(self, initial, final, tone):
        ...
        self._url = 'https://www.yoyochinese.com/audio/pychart/%s%s' %\
    ...
    for initial in INITIALS:
        for final in FINALS:
            for tone in pinyin.TONES:
                ret.append(YoyoLink(initial, final, tone))
    return ret

def is_error(fname):
    # Yoyo Chinese returns an HTML document whenever we request a sound file
    # that does not exist
    return os.stat(fname).st_size == 20004 \
       and magic.from_file(fname).startswith('HTML')

if __name__ == '__main__':
    SITE = 'yoyo'
    parser = argparse.ArgumentParser(description='Yoyo Chinese pinyin sounds')
    ...
    if args.download:
        ...
        pinyin.download_files(links, args.datadir, args.prefix, AUDIO_FORMAT, is_error)
    if args.anki:
